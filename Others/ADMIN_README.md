...
For local development, you'll use two files:
- **`.env`**: This file should contain placeholder values and can be committed to Git.
- **`.env.local`**: This file should contain your actual sensitive credentials and will be ignored by Git.

1.  **Create your base `.env` file:**
    Copy the provided `.env.example` to `.env` in your project root:
    ```bash
    cp .env.example .env
    ```
    Fill in the placeholder values in `.env` with generic, non-sensitive information (e.g., `your_db_password_placeholder` can become `admin` for local testing).

2.  **Create your local `.env.local` file:**
    Copy the `.env` file to `.env.local` in your project root:
    ```bash
    cp .env .env.local
    ```
    Now, update the `.env.local` file with your **actual database passwords and Twilio API keys**. These are your sensitive credentials.
...
DB_USER=postgres
DB_PASSWORD=admin # Your actual local DB password

# Session
SESSION_SECRET=siva-medicals-super-secret-key-change-in-production # Your actual local secret

# Twilio (optional for WhatsApp)
TWILIO_ACCOUNT_SID=ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx # Your actual Twilio SID
TWILIO_AUTH_TOKEN=your_actual_twilio_auth_token # Your actual Twilio Auth Token
TWILIO_WHATSAPP_NUMBER=+14155238886 # Your Twilio sandbox number
WEBSITE_WHATSAPP_NUMBER=9245464648 # Your actual website WhatsApp number
